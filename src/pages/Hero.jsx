...
export default function Hero() {
  ...
  return <main className="hero-landing">
    ...
    <section className="hero-landing-content">
      <div className="hero-landing-kicker"><span>✦</span> 30초 몬스터 헌팅 <span>✦</span></div>
      <div className="hero-copy-block">
        <h1>MONSTER<span>POP</span></h1>
        <p>잡고, 콤보를 쌓고,<br /><strong>랭킹을 정복하세요.</strong></p>
      </div>

      <div className="hero-actions">
        <button className="hero-start-button" onClick={() => enter(user ? '/home' : '/login')}>
          <span>{user ? '사냥터로 돌아가기' : '지금 사냥 시작하기'}</span>
          <b aria-hidden="true">→</b>
        </button>
        {!user && <button className="hero-login-link" onClick={() => enter('/login')}>
