...
export default function Community() {
  ...
  async function submit() {
    ...
    // 방금 쓴 글도 아바타가 바로 보여야 한다. avatar_url 을 빼면 새로고침 전까지 기본 아바타로 뜬다.
    setPosts((items) => [{ category: '자유', post_likes: [], ...inserted, profiles: { nickname: profile?.nickname, avatar_url: profile?.avatar_url } }, ...items])
  ...
  return <main className="page community-page">
    ...
    {/* 앞의 둘은 분류가 아니라 보기 방식이라 먼저 두고, 분류는 자유 → 공략 → 기록 인증 순. */}
    <div className="filter-chips community-categories">{['전체', '인기', '자유', '공략', '기록 인증'].map((item) => <button key={item} className={activeCategory === item ? 'active' : ''} onClick={() => setActiveCategory(item)}>{item}</button>)}</div>
    {loadError && <div className="notice notice-error">{loadError}</div>}
    {loading ? <div className="empty-state"><span className="loader" />게시글을 불러오는 중...</div> : visiblePosts.length === 0 ? <div className="empty-state"><span className="empty-icon"><Icon name="chat" /></span><h3>{posts.length ? '조건에 맞는 이야기가 없어요' : '첫 이야기를 기다리고 있어요'}</h3><p>{posts.length ? '검색어나 카테고리를 바꿔보세요.' : <>공략이나 멋진 기록을 공유해보세요.<br />아래 + 버튼으로 첫 글을 작성할 수 있어요.</>}</p></div> : <section className="post-list">{visiblePosts.map((post, index) => <article key={post.id} className="post-item" onClick={() => navigate(`/community/${post.id}`)}><div className="post-head"><Avatar avatarUrl={post.profiles?.avatar_url} size="small" /><div><strong>{post.profiles?.nickname || '익명 헌터'}</strong><small>{timeAgo(post.created_at)}</small></div><span className="pill post-category">{post.category || '자유'}</span>{index === 0 && activeCategory === '인기' && <span className="pill pill-best">BEST</span>}</div><h2>{post.title}</h2><p>{post.content.length > 90 ? `${post.content.slice(0, 90)}…` : post.content}</p><div className="post-footer"><span className="post-stats"><button className={post.post_likes?.some((like) => like.user_id === user.id) ? 'liked' : ''} onClick={(event) => toggleLike(event, post)} aria-label="좋아요">♥ {post.post_likes?.length || 0}</button><span className="post-comment-count"><Icon name="chat" size={14} /> {post.comment_count ?? 0}</span></span><span className="post-more">이야기 보기 <b>›</b></span></div></article>)}</section>}
    <button className="fab" onClick={() => { sound.button(); setShowWrite(true) }} aria-label="글쓰기"><Icon name="plus" size={25} /></button>
    {showWrite && <div className="modal-overlay" onClick={() => setShowWrite(false)}><section className="modal" onClick={(e) => e.stopPropagation()}><div className="modal-handle" /><div className="modal-title"><div><span className="overline">NEW STORY</span><h2>새 글 작성</h2></div><button className="text-button" onClick={() => setShowWrite(false)}>닫기</button></div><div className="field"><label htmlFor="post-category">분류</label><select id="post-category" value={category} onChange={(e) => setCategory(e.target.value)}><option>자유</option><option>공략</option><option>기록 인증</option></select></div><div className="field"><label htmlFor="post-title">제목</label><input id="post-title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="어떤 이야기를 나눌까요?" maxLength={60} /></div><div className="field"><label htmlFor="post-content">내용</label><textarea id="post-content" value={content} onChange={(e) => setContent(e.target.value)} rows={6} placeholder="공략, 기록, 몬스터 이야기 모두 좋아요." /></div>{error && <div className="notice notice-error">{error}</div>}<div className="btn-row"><button className="btn btn-secondary" onClick={() => setShowWrite(false)}>취소</button><button className="btn btn-primary" onClick={submit} disabled={busy}>{busy ? '등록 중...' : '게시하기'}</button></div></section></div>}
    <BottomNav />
  </main>
}
