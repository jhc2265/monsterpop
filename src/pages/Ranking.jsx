...
function Podium({ row, rank, me }) { return <article className={`podium-item rank-${rank} ${me ? 'me' : ''}`}><div className="podium-medal"><Avatar row={row} /><img className="podium-frame" src={`/images/ranking/frame-${rank === 1 ? 'gold' : rank === 2 ? 'silver' : 'bronze'}.webp`} alt="" /><b>{rank}</b></div><strong>{row.profiles?.nickname || '익명 헌터'}</strong><span>{row.score.toLocaleString()}점</span><small>최고 콤보 {row.max_combo || 0}</small></article> }
function PodiumPlaceholder({ rank }) { return <article className={`podium-item rank-${rank} placeholder`}><div className="podium-medal"><Avatar /><img className="podium-frame" src={`/images/ranking/frame-${rank === 1 ? 'gold' : rank === 2 ? 'silver' : 'bronze'}.webp`} alt="" /><b>{rank}</b></div><strong>도전자를 기다려요</strong><span>-</span></article> }
