...
export default function Ranking() {
  ...
  return <main className="page ranking-page">
    ...
    {loading ? <div className="empty-state"><span className="loader" />랭킹을 집계하는 중...</div> : rows.length === 0 ? <Empty /> : <>
      ...
      {myRank >= 0 && <section className="my-rank-card"><div className="my-rank-head"><span>MY RANK</span><strong>{myRank + 1}위</strong></div><RankRow row={rows[myRank]} rank={myRank + 1} me /><div className="my-rank-stats"><div><small>최고 점수</small><strong>{myBest.toLocaleString()}</strong><em>{myDelta === null ? '첫 기록에 도전' : <>이전 대비 <b className={myDelta >= 0 ? '' : 'down'}>{myDelta >= 0 ? '▲' : '▼'} {Math.abs(myDelta)}%</b></>}</em></div><i /><div><small>전체 순위</small><strong>{myRank + 1}위</strong><em>전체 {rows.length}명</em></div></div></section>}
...
function Avatar({ row }) { return <div className="avatar"><img src={row?.profiles?.avatar_url || '/images/ranking/default-hunter.png'} alt="" /></div> }
