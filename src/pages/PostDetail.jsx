...
export default function PostDetail() {
  ...
  useEffect(() => { load() }, [id])
  async function load() { setLoading(true); const { data } = await supabase.from('posts').select('id, title, content, created_at, user_id, profiles(nickname)').eq('id', id).single(); setPost(data); await loadComments(); setLoading(false) }
